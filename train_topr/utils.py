...
from topr_loss import TOPRLoss
...
def make_topr_model(cfg, train_env, eval_env, device="cpu"):
    model_cfg = cfg.model

    in_keys = ["observation"]
    action_spec = train_env.action_spec_unbatched
    actor_net = make_topr_module_state(model_cfg, eval_env)
    ...
    model = torch.nn.ModuleList([actor]).to(device)
    # init nets
...
def make_topr_module_state(model_cfg, proof_environment):
